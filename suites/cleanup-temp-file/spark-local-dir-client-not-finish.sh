...
$SPARK_HOME/bin/spark-submit --conf spark.master=spark://$SYM_MASTER_HOST:$global_master_port --deploy-mode client --class job.submit.control.submitSleepTasks $SAMPLE_JAR 3 10000 &>> $global_case_log_dir/tmpOut &
appPID=$!
...
echo "$global_case_name - check dir"
...
echo "$global_case_name - cleanup status: $cleanup_stat"
echo "$global_case_name - reason: $cleanup_reason"
