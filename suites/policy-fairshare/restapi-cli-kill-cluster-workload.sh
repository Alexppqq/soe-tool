...
$SPARK_HOME/bin/spark-submit --conf spark.master=spark://$SYM_MASTER_HOST:7077 --deploy-mode cluster  --class job.submit.control.submitSleepTasks $SAMPLE_JAR 3 20000 &>> $val_case_log_dir/tmpOut
sleep 10
...
curl -d "" http://$SYM_MASTER_HOST:6066/v1/submissions/kill/$drivername 1> $val_case_log_dir/tmpKill 2>> /dev/null
killstatus=`ca_get_kill_driver_status $val_case_log_dir/tmpKill`
echo $killstatus
